...
import { Hammer, Search, Star, AlertCircle, Loader2, X, ShieldCheck } from 'lucide-react'
import { developersAPI } from '../api/client'
...
export default function DevelopersPage() {
  ...
  const fetchDevelopers = async () => {
    ...
    try {
      ...
      const { data } = await developersAPI.list(params)
  ...
  const handleOpenRateModal = (developer) => {
    if (!isAuthenticated) {
      alert('Please log in or register to submit a review.')
      return
    }
    setRatingTarget(developer)
    setRatingVal(5)
    setCommentVal('')
    setReviewError(null)
  }
  ...
  const handleRateSubmit = async (e) => {
    ...
    setReviewError(null)
    try {
      await developersAPI.rate(ratingTarget.id, {
        rating: ratingVal,
        comment: commentVal,
      })
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <div className="relative z-10 max-w-2xl">
            ...
            >
              ...
              <span>Verified Estate Developers</span>
            </div>
            <h1 className="text-3xl sm:text-5xl font-black mb-4 tracking-tight leading-tight text-white">
              Verified Real Estate Developers
            </h1>
            <p className="text-base sm:text-lg font-normal leading-relaxed text-indigo-100">
              Browse serviced layouts, structural plans, and off-plan properties built by reputable estate developers in Nigeria. Connect directly with development firms without intermediate fees.
            </p>
          </div>
        </div>
        ...
        <div className="bg-surface rounded-2xl p-4 shadow-card border border-border mb-8 flex flex-col md:flex-row gap-4 items-center justify-between">
          <form onSubmit={handleSearchSubmit} className="relative flex-1 w-full">
            ...
            <input
              type="text"
              placeholder="Search company name or developer..."
              value={search}
        ...
        {loading ? (
          ...
          <div className="bg-surface rounded-3xl p-12 text-center max-w-xl mx-auto border border-border shadow-sm my-8">
            ...
            <p className="text-text-muted text-sm mb-6">
              We couldn't find any developers matching your search query. Try broadening your search.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-md bg-surface rounded-2xl shadow-xl border border-border overflow-hidden">
            <div className="p-6 border-b border-border flex items-center justify-between">
              <h3 className="text-lg font-bold text-text-primary">Review & Rate Developer</h3>
            ...
            <form onSubmit={handleRateSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <textarea
                  rows={4}
                  required
                  placeholder="Share your experience dealing with this developer..."
                  value={commentVal}
